...
### Abort after the first issue

By default, Zod keeps validating to collect additional continuable issues. To stop after the first issue, pass `abortEarly: true` to `parse`, `safeParse`, and related methods:

```ts
const result = z.object({
  name: z.string(),
  age: z.number(),
}).safeParse({ name: 42, age: "invalid" }, { abortEarly: true });
